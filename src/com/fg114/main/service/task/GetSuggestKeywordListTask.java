...
public class GetSuggestKeywordListTask extends BaseTask {
	...
	public GetSuggestKeywordListTask(Context context, int startIndex) {
		super(context);
	...
	public JsonPack getData() throws Exception {
		...
		if (vo != null && !vo.isExpired()) {
			...
			dto = CommonTypeListDTO.toBean(jp.getObj());
		} else {
			jp = ServiceRequest.getSuggestKeywordList(keywords, Settings.DEFAULT_RES_AND_FOOD_PAGE_SIZE, startIndex);
			// 如果结果正确，存入缓存
			if (jp.getRe() == 200 && jp.getObj() != null) {
				dto = CommonTypeListDTO.toBean(jp.getObj());
				if (dto != null && dto.getList() != null && dto.getList().size() > 0) { // 有结果才缓存
