...
public abstract class HttpHeaderFactory {
	...
	private String generateAuthSignature(final String method, WeiboParameters signatureParams, 
        ...
        .append("&");
        base.append(encode(encodeParameters(signatureParamsSort(signatureParams), "&", false)));
        String oauthBaseString = base.toString();
		return oauthBaseString;
	}
	//fixbug，排序参数
	private WeiboParameters signatureParamsSort(WeiboParameters params){
		TreeSet<String> ss=new TreeSet<String>();
		WeiboParameters newParams=new WeiboParameters();
		for(int i=0;i<params.size();i++){
			ss.add(params.getKey(i));			
		}
		String[] keys=ss.toArray(new String[]{});
		for(int i=0;i<keys.length;i++){
			newParams.add(keys[i],params.getValue(keys[i]));			
		}
		return newParams;
